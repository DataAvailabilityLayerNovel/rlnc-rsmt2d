...
func (m *mockKZG) Combine(commits []PieceCommitment, coeffs []byte) (ColumnCommitment, error) {
	if len(commits) == 0 {
		return ColumnCommitment(make([]byte, 32)), nil
	}
	result := make([]byte, 32)
	...
	return ColumnCommitment(result), nil
}

func (m *mockKZG) CombineProofs(proofs []OpeningProof, coeffs []byte) (OpeningProof, error) {
	// Cast proofs to []PieceCommitment to call Combine
	commits := make([]PieceCommitment, len(proofs))
	for i, p := range proofs {
		commits[i] = PieceCommitment(p)
	}
	comb, err := m.Combine(commits, coeffs)
	if err != nil {
		return nil, err
	}
	return OpeningProof(comb), nil
}

// Verify luôn trả true — unit test chỉ kiểm tra luồng dữ liệu, không kiểm tra mật mã.
func (m *mockKZG) Verify(commit []byte, row int, data []byte, proof OpeningProof) bool {
	return true
}
...
func makeMockProofs(k int) []OpeningProof {
	proofs := make([]OpeningProof, k)
	for i := range proofs {
		proofs[i] = OpeningProof(randBytes(32))
	}
	return proofs
}

// storeCell giúp tạo và khởi tạo một StorageNode từ dữ liệu cell.
func storeCell(t *testing.T, row, col, k int, cellData []byte, proofs []OpeningProof, kzg KZGProvider) *StorageNode {
	t.Helper()
...
func marshalOpeningProof(t *testing.T, proof bls12381kzg.OpeningProof) OpeningProof {
	t.Helper()
	var out bytes.Buffer
	_, err := proof.WriteTo(&out)
	require.NoError(t, err)
	return OpeningProof(out.Bytes())
}

func makeGnarkFragmentCommitmentsAndProofs(t *testing.T, kzg *GnarkKZG, row int, fragments [][]byte) ([]PieceCommitment, []OpeningProof) {
	t.Helper()
	commits := make([]PieceCommitment, len(fragments))
	proofs := make([]OpeningProof, len(fragments))
...
func TestUnit_VerifyPiece(t *testing.T) {
	...
	recv := NewRecipientManager(k, kzg)
	pubComm := ColumnCommitment(make([]byte, 32))
